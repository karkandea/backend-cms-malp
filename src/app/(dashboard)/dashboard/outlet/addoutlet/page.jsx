  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      const [logoUrl, bannerUrl] = await Promise.all([
        uploadWithSignedUrl(logo.file, "logo"),
        uploadWithSignedUrl(banner.file, "banner"),
      ]);

      const response = await fetch("/api/v1/outlets", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok || !payload?.data?.id) {
        ...
        if (rootIssue) {
          ...
          setSubmissionError(apiError.message ?? "Gagal menyimpan outlet.");
        }

        throw new Error(apiError.message ?? "Gagal menyimpan outlet.");
      }

      setCreatedOutletId(payload.data.id);
      setIsSubmitting(false);
      setIsSuccessOpen(true);
    } catch (error) {
      console.error("[AddOutlet] submit error", error);
      setSubmissionError(error.message ?? "Gagal menyimpan outlet.");
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <section className="space-y-6">
      <header className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-semibold text-slate-900">Add Outlet</h1>
      </header>
