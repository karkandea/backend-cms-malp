...
import OutletFacilitySkeleton from "./skeleton";
import OutletFacilityView from "./view";
...
async function FacilityContent({ outletId }) {
  const headerList = await headers();
  const cookieStore = await cookies();
  const baseUrl = resolveBaseUrl(headerList);
  const targetUrl = `${baseUrl}/api/v1/outlets/${encodeURIComponent(outletId)}/facility`;
  ...
  if (!response.ok) {
    console.error("[OutletFacilityPage] fetch error", response.statusText);
    return (
      <section className="space-y-4">
        <Link
          href="/dashboard/outlet"
        ...
        <div className="rounded-3xl border border-amber-200 bg-amber-50 px-4 py-6 text-sm text-amber-700">
          Tidak dapat memuat fasilitas outlet saat ini. Coba lagi nanti.
        </div>
      </section>
    );
  }
  ...
  return (
    <OutletFacilityView
      ...
      outletName={facility.name}
      facility={facility}
    />
  );
}

export default async function OutletFacilityPage({ searchParams }) {
  ...
  if (!id) {
    return (
      <section className="space-y-4">
        ...
        <div className="rounded-3xl border border-rose-200 bg-rose-50 px-4 py-6 text-sm text-rose-600">
          Parameter <code>id</code> wajib ada untuk melihat fasilitas outlet.
        </div>
      </section>
    );
  }
  ...
  return (
    <ToastProvider>
      <Suspense fallback={<OutletFacilitySkeleton />}>
        <FacilityContent outletId={id} />
      </Suspense>
    </ToastProvider>
  );
}
